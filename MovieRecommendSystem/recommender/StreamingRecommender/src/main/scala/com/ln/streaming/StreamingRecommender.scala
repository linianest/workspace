...
object ConnHelper extends Serializable {
  lazy val jedis = new Jedis("mini1")
  lazy val mongoClient = MongoClient(MongoClientURI("mongodb://mini1:27017/recommender"))
}
...
object StreamingRecommender {
  ...
  def main(args: Array[String]): Unit = {
    val config = Map(
      "spark.cores" -> "local[*]",
      "mongo.uri" -> "mongodb://mini1:27017/recommender",
      "mongo.db" -> "recommender",
      "kafka.topic" -> "recommender"
    )

    val sparkConf = new SparkConf().setMaster(config("spark.cores")).setAppName(this.getClass.getName)
